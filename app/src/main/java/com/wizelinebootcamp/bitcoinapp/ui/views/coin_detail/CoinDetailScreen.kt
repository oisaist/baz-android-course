        ...
        content = {
            /*when (orderBook) {
                ...
                is NetworkResponse.Success -> {
                    BidAskList(bidAksModel = orderBook?.data?.payload?.bids ?: listOf())
                }
                else -> {}
            }*/
            Column(
                ...
                CustomSpacer()
                Text(text = "Bids", style = MaterialTheme.typography.h6, modifier = Modifier.fillMaxWidth(), textAlign = TextAlign.Center, fontWeight = FontWeight.Bold)

                CustomSpacer()
                when (orderBook.value) {
                    is NetworkResponse.Loading -> { CustomProgressBar(modifier = Modifier
                        .fillMaxWidth()
                        .padding(vertical = 16.dp)) }
                    is NetworkResponse.Success -> {
                        if (orderBook.value?.data?.payload?.bids.isNullOrEmpty())  EmptyContent(modifier = Modifier.fillMaxWidth())
                        else BidAskList(bidAksModel = orderBook.value?.data?.payload?.bids ?: listOf())
                    }
                    else -> ErrorScreen(
                ...
                CustomSpacer()
                Text(text = "Asks", style = MaterialTheme.typography.h6, modifier = Modifier.fillMaxWidth(), textAlign = TextAlign.Center, fontWeight = FontWeight.Bold)
                ...
                when (orderBook.value) {
                    is NetworkResponse.Loading -> { CustomProgressBar(modifier = Modifier
                        ...
                        .padding(vertical = 16.dp)) }
                    is NetworkResponse.Success -> {
                        if (orderBook.value?.data?.payload?.asks.isNullOrEmpty()) EmptyContent(modifier = Modifier.fillMaxWidth())
                        else BidAskList(bidAksModel = orderBook.value?.data?.payload?.asks ?: listOf())
                        CustomSpacer()
                    }
                    else -> ErrorScreen(
                        modifier = Modifier.fillMaxSize(),
